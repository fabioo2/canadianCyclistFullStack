...
exports.renderGalleryPage = (req, res) => {
    ...
    let getRelatedEvents = `select s.id, s.cc_event_id, s.event_sub_category, e.event_title, year(e.date_from) as year_num from cc_event_subcategory s join cc_event e where s.cc_event_id = e.id and year(e.date_from) = ${pool.escape(year_num)} and e.id = ${pool.escape(event_id)}`;
    ...
    pool.query(getSelectedEvent, (err, result) => {
        ...
        pool.query(getPhotos, (err2, result2) => {
            ...
            pool.query(getRelatedEvents, (err3, result3) => {
                if (err3) {
                    res.redirect('/');
                }
